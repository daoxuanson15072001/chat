...
export class InitDB1680062965475 implements MigrationInterface {
    name = 'InitDB1680062965475'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`room\` (\`id\` int UNSIGNED NOT NULL AUTO_INCREMENT, \`name\` text NOT NULL, \`description\` text NULL, \`type\` enum ('1', '2') NOT NULL DEFAULT '1', \`status\` tinyint NOT NULL COMMENT '0: Inactive, 1: Active.' DEFAULT '1', \`avatar\` varchar(255) NULL, \`created_at\` timestamp(6) NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
